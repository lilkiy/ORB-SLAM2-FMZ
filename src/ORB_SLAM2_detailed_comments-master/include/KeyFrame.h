...
public:
     ...
     */
    std::set<MapPoint*> GetMapPoints();  // 获取该关键帧中的某一个地图点
    /**
     * @brief Get MapPoint Matches 获取该关键帧的MapPoints
     */
    std::vector<MapPoint*> GetMapPointMatches();  // 获取当前关键帧中特征点对应的地图点
    /**
...
protected:
    ...
    std::vector<MapPoint*> mvpMapPoints; //存放当前关键帧观测到的地图点的容器
    ...
    // Covisibility Graph
    // 共视图结构由以下三个成员变量维护
     // 与该关键帧连接（至少15个共视地图点）的关键帧与权重  无顺序
    ...
    // 当前关键帧是否还未加入到生成树,构造函数中初始化为true,加入生成树后置为false
    bool mbFirstConnection;                     // 是否是第一次生成树
    KeyFrame* mpParent;                         // 当前关键帧的父关键帧 （共视程度最高的）
    std::set<KeyFrame*> mspChildrens;           // 存储当前关键帧的子关键帧(只有一个父关键帧 多个子关键帧)
    std::set<KeyFrame*> mspLoopEdges;           // 和当前关键帧形成回环关系的关键帧集合

    // Bad flags
    bool mbNotErase;            ///< 当前关键帧已经和其他的关键帧形成了回环关系，因此在各种优化的过程中不应该被删除 当前关键帧是否具有不被删除的特权
    bool mbToBeErased;          ///< 当前关键帧是否曾被豁免过删除
    bool mbBad;                 ///< 标记是坏帧 初始值为false
