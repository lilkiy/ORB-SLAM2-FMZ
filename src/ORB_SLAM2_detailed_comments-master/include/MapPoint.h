...
public:
    ...
    /**
     * @brief 添加观测 向当前的MapPoint对象中添加一个观测
     * 参数pKF是指向观测该MapPoint对象的关键帧KeyFrame对象的指针，参数idx表示该MapPoint在KeyFrame中的特征点序号
...
protected:
    ...
    // 观测到该MapPoint的KF和该MapPoint在KF中的索引，一个KF会观测到多个地图点
    // map是一个存放键值对的容器
    std::map<KeyFrame*,size_t> mObservations; // mappoint类的成员变量
