...
public:
    ...
    // Tracking states
    ///定义跟踪状态类型为枚举类型
    // 枚举类型由一组具有离散值的常量构成的。在枚举类型中，每个常量都有一个唯一的名称，并且可以在程序中被引用和使用。
    // 在声明时，程序员需要指定每个常量的名称和值
    enum eTrackingState{
    ...
    std::vector<int> mvIniMatches;// 跟踪初始化时前两帧之间的匹配  单目初始化中参考帧与当前帧的匹配关系
    ///在初始化的过程中,保存参考帧中的特征点
    ...
    // 我们将每个帧与它的参考关键帧相关联，并保存它们之间的相对变换。这个过程是为了在程序执行结束时恢复完整相机轨迹而进行的。
    // 当前帧相对于上一参考关键帧的位姿变换
    list<cv::Mat> mlRelativeFramePoses; // 每个帧存储参考关键帧和它们之间的相对变换 
    ///参考关键帧
...
protected:
    ...
    /** @brief 单目输入的时候生成初始地图 */  // 单目初始化成功后建立初始局部地图
   ...
   /**
     ...
     * @brief 创建新的关键帧
     *
     * 对于非单目的情况，同时创建新的MapPoints（仅针对双目或RGBD相机会生成新的地图点）
     */
    ...
    ///局部关键帧集合 局部关键帧列表,初始化成功后向其中添加局部关键帧
    // 初始化得到的地图点都作为局部地图点
    std::vector<KeyFrame*> mvpLocalKeyFrames;
    ///局部地图点的集合 局部地图点列表,初始化成功后向其中添加局部地图点
