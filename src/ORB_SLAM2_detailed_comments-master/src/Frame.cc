     ...
     mpReferenceKF(static_cast<KeyFrame*>(NULL))
{
    ...
    // ORB extraction 提取双目图像特征点 左右目分两个线程并行提取
	...
	//mvKeys中保存的是左图像中的特征点（去畸变之前），这里是获取左侧图像中特征点的个数
    N = mvKeys.size();
    ...
    if(mbInitialComputations) // 第一次调用帧构造函数时 初始化变量默认为true
    {
		//计算去畸变后图像的边界
        // 对图像作去畸变处理后图像的边界会变化吗
        // 因为去畸变处理通常会对图像进行裁剪或者拉伸，以消除图像中的畸变。在这个过程中，可能会出现图像边界的丢失或者扭曲，从而导致图像边界的变化。
        ComputeImageBounds(imLeft);

		// 表示一个图像像素相当于多少个图像网格列（宽）  每个网格的宽度/高度
...
/**
 * @brief 将解释orb-slam2中的函数AssignFeaturesToGrid()
 * 
 */
void Frame::AssignFeaturesToGrid()
{
    ...
    // 用于计算需要为每个网格单元预留多少个关键点的空间
    // 乘以0.5，即将每个网格单元预留的空间减少一半。这样做是为了避免在网格单元中存储太多的关键点，从而导致内存占用过高。
    // 实际上，为每个网格单元预留的空间大小也取决于关键点的密度和分布情况。
    // 在实际使用时，可以根据具体情况对预留空间的大小进行调整，以达到最佳的性能和鲁棒性。
...
void Frame::UndistortKeyPoints()
{
    ...
    cv::Mat mat(N,2,CV_32F);  // 用于保存特征点
	//遍历每个特征点，并将它们的坐标保存到矩阵中
    for(int i=0; i<N; i++)
    {
		//然后将这个特征点的横纵坐标分别保存
        mat.at<float>(i,0)=mvKeys[i].pt.x;  //mvKeys存放去畸变前的特征点
...
void Frame::ComputeStereoMatches()
{
    ...
    // mvuRight存储右图匹配点索引
    // mvDepth存储特征点的深度信息
    // 创建了一个名为 "mvuRight" 的浮点型向量，其长度为 N，初始化为 -1.0f。具体来说，该向量包含 N 个浮点数元素，每个元素的初始值都为 -1.0f.
    // 单目特征点这两个变量的值默认-1
	mvuRight = vector<float>(N,-1.0f);
    mvDepth = vector<float>(N,-1.0f);

	// orb特征相似度阈值  -> mean ～= (max  + min) / 2
    // ORBmatcher::TH_HIGH 和 ORBmatcher::TH_LOW 是 ORB 特征匹配中的两个阈值，其中 TH_HIGH 表示高阈值，TH_LOW 表示低阈值，
    //它们分别用于区分强匹配和弱匹配。具体来说，当两个特征点之间的 Hamming 距离小于 TH_HIGH 时，它们被认为是强匹配；
    //当 Hamming 距离大于 TH_HIGH，但小于 TH_LOW 时，它们被认为是弱匹配。只有强匹配才会被用于后续的位姿估计和地图构建等过程，而弱匹配则被舍弃。
    // 阈值设置在强匹配和弱匹配中间，当小于这个阈值时 进入精匹配阶段
    ...
    for(int iR = 0; iR < Nr; iR++) {
        ...
        // mvScaleFactors 存储了不同层金字塔之间的尺度因子。
        // mvKeysRight[iR].octave 是 ORB-SLAM2 中右图像中第 iR 个特征点所在的金字塔层数。它的取值范围为 0 到 N-1，其中 N 是金字塔的层数。
        // mvScaleFactors 用于获取该层的尺度因子
        // 在 ORB 特征中，每个特征点会被分配到一个金字塔层级，以提高其对不同尺度下的变化具有鲁棒性。金字塔的层数越高，特征点所在的图像尺度越小，特征点对尺度变化的鲁棒性也越强。
        // 
        // r表示索引为iR的右图特征点在其所在图像金字塔相应图像层上的像素偏移量
        // 理论上双目相机左右目特征点对的x坐标不同，y坐标相同，但y坐标实际可能会出现大小为r的偏移

        // 计算右图特征点在行方向上，可能的偏移范围
        const float r = 2.0f * mvScaleFactors[mvKeysRight[iR].octave];
        const int maxr = ceil(kpY + r); //ceil 函数是向上取整
        const int minr = floor(kpY - r); //floor 函数是向下取整

        // 将特征点ir保证在可能的行号中
        for(int yi=minr;yi<=maxr;yi++)
        //将特征点按照行号范围分组，每一组包含了一些特征点，这些特征点在图像中的行号都在这个范围内。
        // 在 ORB-SLAM2 中，采用的是按行号分组的方式，将特征点分配到不同的行中，以便后续的双目立体匹配。
            vRowIndices[yi].push_back(iR); // 把特征点诉索引存入可能的行方向偏移范围内的行号中
    }
    ...
    const float minZ = mb;  // 场景中物体到相机的最近距离 这里取值为基线长度
    ...
    for(int iL=0; iL<N; iL++) {
		...
		const cv::KeyPoint &kpL = mvKeys[iL]; //iL为左图特征点索引
        const int &levelL = kpL.octave; // 获取左图特征点所在的金字塔层级
        const float &vL = kpL.pt.y;  // y坐标 即行号
        const float &uL = kpL.pt.x;  // x坐标即列号

        // 获取左图特征点il所在行，以及在右图对应行中可能的匹配点
        // vRowIndices 是一个二维向量，每个元素 vRowIndices[i] 表示右图中行号为 i 的特征点集合，其中存储的是这些特征点在右图中的索引。
        // 在左图中，当前处理的特征点的行号是 vL，因此可以通过 vRowIndices[vL] 来获取右图中与之可能匹配的特征点的集合。
        // 由于 vRowIndices[vL] 中存储的是特征点在右图中的索引，因此可以直接将其赋值给 vCandidates，表示这些特征点是当前左图特征点在右图中可能的匹配点。
        // 这样做的好处是避免了对右图中的所有特征点进行遍历匹配，只考虑了可能匹配的特征点，提高了匹配效率。
        const vector<size_t> &vCandidates = vRowIndices[vL];
        if(vCandidates.empty()) continue;  // 在右图相应行号上没有特征点

        // 计算理论上的最佳搜索范围  x坐标方向上的搜索范围 即只搜索可能匹配范围内的特征点进行匹配
        // 视差d=uL-uR minU和maxU相当于左图该特征点在同一行上，右图可能匹配特征点的x坐标范围
		...
		// 初始化最佳相似度，用最大相似度，以及右图中最佳匹配点索引
        // bestDist 的作用是用于记录当前左图特征点在右图中的最佳匹配的汉明距离，初始化为 TH_HIGH 表示暂时还没有找到合适的匹配
        int bestDist = ORBmatcher::TH_HIGH;
        size_t bestIdxR = 0;
        // 这段代码的作用是获取左图中第 iL 个特征点的描述子(的引用)，其中 mDescriptors 是一个存储所有特征点描述子的矩阵，每一行表示一个特征点的描述子。
        // 具体来说，mDescriptors.row(iL) 表示获取矩阵 mDescriptors 的第 iL 行，即表示第 iL 个特征点的描述子.
        ...
        for(size_t iC=0; iC<vCandidates.size(); iC++) {
            ...
            if(uR >= minU && uR <= maxU) {
                ...
                if( dist<bestDist ) {
                    bestDist = dist;
                    bestIdxR = iR;  // 右图中与左图特征点 iL 匹配的最佳特征点的索引
                }
            }
        }
        ...
        if(bestDist<thOrbDist) {
            // 如果刚才匹配过程中的最佳描述子距离小于给定的阈值，则认为匹配成功，进入精匹配阶段
            // 计算右图特征点x坐标和对应的金字塔尺度
            const float uR0 = mvKeysRight[bestIdxR].pt.x;  // 获取右图中与左图特征点 iL 匹配的最佳特征点的 x 坐标 uR0，用于后续的位姿估计和三角化等计算中。
            const float scaleFactor = mvInvScaleFactors[kpL.octave]; // 获取左图中特征点 iL 所在的图像金字塔层数 kpL.octave，并计算该层的尺度因子 scaleFactor。
            
            // 尺度缩放后的左右图特征点坐标
            // 将左右图像中特征点的坐标都缩放到同一金字塔层上，便于进行后续的图像窗口匹配。
            // 将左图中的特征点坐标和右图中的最佳匹配点坐标都按照对应的金字塔层的尺度因子进行缩放
            // round 函数是将浮点数四舍五入为最接近的整数，这里是将缩放后的坐标四舍五入为整数。
            ...
            // 通过 mpORBextractorLeft->mvImagePyramid[kpL.octave] 获取特征点所在金字塔层的图像
            // 然后使用 rowRange 和 colRange 函数分别裁剪出以 scaledvL-w 为起点、w2+1 为高度的行区域和以 scaleduL-w 为起点、w2+1 为宽度的列区域。
            // 实现了左图中以特征点 kpL 为中心的图像窗口的提取
            cv::Mat IL = mpORBextractorLeft->mvImagePyramid[kpL.octave].rowRange(scaledvL-w,scaledvL+w+1).colRange(scaleduL-w,scaleduL+w+1);
            IL.convertTo(IL,CV_32F); // IL为在左图中提取的图像块 存储在矩阵 IL 中的图像窗口数据类型转换为 32 位浮点型
            
            // 图像块均值归一化，降低亮度变化对相似度计算的影响
            // 首先通过 IL.at<float>(w,w) 获取图像窗口中心像素点的值，然后通过 cv::Mat::ones(IL.rows,IL.cols,CV_32F) 创建一个与图像窗口 IL 相同尺寸的矩阵，且矩阵中的所有元素均为 1。
            // 接着将该矩阵乘以中心像素点的值，得到一个尺寸与 IL 相同的矩阵，每个元素的值均为中心像素点的值。最后，将该矩阵从 IL 中减去，即可得到均值化后的图像窗口 IL。
            // 该操作可以去除图像窗口的平均值，从而减小光照等因素对图像匹配的干扰，提高匹配精度。
			...
			// 初始化存储图像块相似度
            vector<float> vDists;  //存放SAD值
            vDists.resize(2*L+1);  // 窗口每滑动一个单位存储一个图像相似度
			...
			// 判断搜索是否越界
            // 如果超出边界，则当前窗口内的像素不足以和左图像中的ORB特征点进行匹配，因此需要放弃该点的匹配。
            // mpORBextractorRight->mvImagePyramid[kpL.octave].cols是右图像对应金字塔层数的宽度。
            if(iniu<0 || endu >= mpORBextractorRight->mvImagePyramid[kpL.octave].cols)
                continue;

			// 在搜索范围内从左到右滑动，并计算图像块相似度 每次滑动距离是incR
            for(int incR=-L; incR<=+L; incR++) {

                // 提取右图中，以特征点(scaleduL,scaledvL)为中心, 半径为w的图像快patch
                cv::Mat IR = mpORBextractorRight->mvImagePyramid[kpL.octave].rowRange(scaledvL-w,scaledvL+w+1).colRange(scaleduR0+incR-w,scaleduR0+incR+w+1);
                IR.convertTo(IR,CV_32F); // 转换为浮点数类型
                
                // 图像块均值归一化，降低亮度变化对相似度计算的影响
                IR = IR - IR.at<float>(w,w) * cv::Mat::ones(IR.rows,IR.cols,CV_32F);
                
                // sad 计算，值越小越相似
                // 计算的是两个大小相同的矩阵IL和IR的L1范数，即差的绝对值之和。它用于评估这两个矩阵之间的差异，即匹配度
                float dist = cv::norm(IL,IR,cv::NORM_L1);

                // 统计最小sad和偏移量  相当于这里在右图中确定了一个最佳匹配patch
                if(dist<bestDist) {
                    bestDist = dist;
                    bestincR = incR;  // 在右图中滑动窗口偏移量为多少时，取到的图像patch ，计算得到的SAD最小
                }

                //L+incR 为refine后的匹配点列坐标(x)
                vDists[L+incR] = dist; 	 // 窗口每滑动一个单位存储一个图像相似度
            }
            ...
            const float deltaR = (dist1-dist3)/(2.0f*(dist1+dist3-2.0f*dist2));//用于亚像素级别匹配的位移量
            // deltaR 的计算结果将被用于调整右图像中特征点的位置，以实现更精确的亚像素级别匹配。

            // 亚像素精度的修正量应该是在[-1,1]之间，否则就是误匹配
            if(deltaR<-1 || deltaR>1)
                continue;
            
            // 根据亚像素精度偏移量delta调整最佳匹配索引  调整右图中最佳匹配的x坐标
    ...
    sort(vDistIdx.begin(),vDistIdx.end()); // 默认升序
    const float median = vDistIdx[vDistIdx.size()/2].first;
    const float thDist = 1.5f*1.4f*median;

    // 倒着比较，即从SAD大的开始比较，超过这个阈值的距离将被视为离群点
    // 删除离群点的过程是通过遍历排序后的距离向量来实现的。对于每个距离值，如果它超过了距离阈值，则将对应的匹配点标记为离群点，
    // 该点的2D坐标将被设置为(-1,-1)，然后从匹配点集中删除这个点。
    // 在双目视觉SLAM中，它用于筛选出视差范围不合理的匹配点，以避免在三角化时产生错误的深度估计。
    for(int i=vDistIdx.size()-1;i>=0;i--) {
        if(vDistIdx[i].first<thDist)
            break;
        else {
			// 误匹配点置为-1，和初始化时保持一直，作为error code
            // 将左图特征点索引对应的深度值和右图中对应点的横坐标设置为-1
...
void Frame::ComputeStereoFromRGBD(const cv::Mat &imDepth)	//参数是深度图像
{
    ...
    mvuRight = vector<float>(N,-1); // 将容器中的N个元素初始化为-1
    mvDepth = vector<float>(N,-1);
    ...
    for(int i=0; i<N; i++)
    {
		...
		//获取校正前和校正后的特征点
        // mvKeys 存储的是原始图像中检测到的关键点，而 mvKeysUn 存储的是经过去畸变处理后的关键点
        ...
        // 按照 (row, col) 的顺序来表示像素坐标，即先行后列。因此，v 表示行坐标，u 表示列坐标，按照 (v, u) 的顺序来表示像素坐标    
        ...
        {
			//那么就保存这个点的深度
            // 一般情况下，深度图像中的像素点深度值都是大于0的，因为深度值是指从相机到场景中物体表面的距离，而物体表面距离相机为0是不可能的
            // 在实际的应用中，由于深度图像获取的误差或者噪声，有时会出现一些深度值为0或者负数的像素点。
            // 这些像素点通常是指该像素点的深度值无法准确地被获取到，或者是获取到的深度值不符合实际情况。
            // 在处理深度图像时，一般需要对这些深度值为0或者负数的像素点进行特殊处理，比如将其剔除或者赋予一个默认值，以避免对后续的计算产生影响。
            mvDepth[i] = d;
			//根据这个点的深度计算出等效的、在假想的右图中的该特征点的横坐标
			//TODO 话说为什么要计算这个嘞，计算出来之后有什么用?可能是为了保持计算一致
            mvuRight[i] = kpU.pt.x-mbf/d; // 在右图中对应特征点的横坐标
        }//如果获取到的深度点合法
    }//开始遍历彩色图像中的所有特征点
    /** </ul> */
}
