                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    mvMatches12.clear(); // 容器清空
	// 预分配空间，大小和关键点数目一致mvKeys2.size()
    mvMatches12.reserve(mvKeys2.size());

    // 记录参考帧1中的每个特征点在当前帧中是否有匹配的特征点
    ...
    {
		//vMatches12[i]解释：i表示帧1中关键点的索引值，vMatches12[i]的值为帧2的关键点索引值
        //没有匹配关系的话，vMatches12[i]值为 -1，即第一帧图像在第二帧图像中没有找到匹配的关键点
        if(vMatches12[i]>=0)
        {
			//mvMatches12 中只记录有匹配关系的特征点对的索引值
            //i表示帧1中关键点的索引值，vMatches12[i]的值为帧2的关键点索引值
            mvMatches12.push_back(make_pair(i,vMatches12[i]));  // 存放特征点匹配关系
			//标记参考帧1中的这个特征点有匹配关系
            mvbMatched1[i]=true;  //表示参考帧中的特征点在当前帧中有匹配关系
        }
        else
			//标记参考帧1中的这个特征点没有匹配关系
            mvbMatched1[i]=false;
    ...
    // Step 2 在所有匹配特征点对中随机选择8对匹配特征点为一组，用于估计H矩阵和F矩阵
    // 共选择 mMaxIterations (默认200) 组  RANSAC迭代次数
    ...
    {
		//迭代开始的时候，所有的点都是可用的
        vAvailableIndices = vAllIndices;  // 存放特征点对的索引
        ...
        for(size_t j=0; j<8; j++)
        {
            ...
            // mvSets是二维容器，第一维存放ransac迭代次数 第二维存放随机选择的8个特征点对的索引
            mvSets[it][j] = idx;  // 每一次ransac迭代都要选8个特征点对

            // 避免随机选到同一个点
            // 由于这对点在本次迭代中已经被使用了,所以我们为了避免再次抽到这个点,就在"点的可选列表"中,
            // 将这个点原来所在的位置用vector最后一个元素的信息覆盖,并且删除尾部的元素
            // 这样就相当于将这个点的信息从"点的可用列表"中直接删除了
            // vAvailableIndices.back()表示获取向量vAvailableIndices的最后一个元素，而pop_back()则表示删除向量的最后一个元素
            // 将向量（vector）vAvailableIndices中的第randi个元素和最后一个元素进行交换，并将最后一个元素弹出向量，即删除向量的最后一个元素。
    ...
    // 创建一个名为"threadF"的新线程，来执行一个名为"FindFundamental"的成员函数，这个函数属于"Initializer"类
    // 在这个函数的执行过程中，它将会使用三个参数：vbMatchesInliersF，SF，F。
    // vbMatchesInliersF和SF是引用类型，也就是说，它们会传入函数并在函数中被修改
    // 这个函数的目的是在输入的匹配点集中计算基础矩阵，并将结果保存在“F”中。
    thread threadF(&Initializer::FindFundamental,this,ref(vbMatchesInliersF), ref(SF), ref(F));
    // Wait until both threads have finished
	//等待两个计算线程结束
    threadH.join(); // 等待"threadH"线程的执行结束，然后才会继续往下执行当前线程。
    threadF.join();
 ...
 * 
 * @param[in & out] vbMatchesInliers          标记是否是外点  包含了所有特征点对的内点外点标记
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    // 计算F矩阵最少只需要7对匹配点,计算H矩阵最少只需要4对匹配点
	//下面进行每次的RANSAC迭代
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        }//读取8对特征点的归一化之后的坐标,存储在容器vPn1i和vPn2i中
        ...
        // 恢复H矩阵的原始尺度
        H21i = T2inv*Hn*T1;  // 解除归一化  H21i是对应于原始数据的H矩阵，Hn是对应于归一化数据的H矩阵
		//然后计算逆
        H12i = H21i.inv();

        // Step 4 利用重投影误差为当次RANSAC的结果评分
        //mSigma:方差默认为1 返回H矩阵的得分
...
void Initializer::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
{
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // Step 4 利用点到极线的距离误差为当次RANSAC的结果评分
    ...
    const vector<cv::Point2f> &vP2) //归一化后的点, in current frame
{
    ...
    cv::Mat A(2*N,				//行，注意每一个点的数据对应两行
			  9,				//列
			  CV_32F);      	//float数据类型 32位浮点数

	// 构造矩阵A，将每个特征点添加到矩阵A中的元素
    // 将每个归一化后的特征点对应的两个约束添加到矩阵A
    for(int i=0; i<N; i++)
    {
		//获取特征点对的像素坐标
    ...
    // 由N个特征点构造矩阵A ，相当于构建了一个超定方程组，Ah=0，通过SVD分解求其最小二乘解
    cv::Mat u,w,vt;

	//使用opencv提供的进行奇异值分解的函数
    // SVD 是一种线性代数中的分解方法，将一个矩阵分解为三个矩阵的乘积，分别为左奇异矩阵、奇异值矩阵和右奇异矩阵。可用于图像压缩降噪等。
    ...
    return vt.row(8).reshape(0, 			//转换后的通道数，这里设置为0表示是与前面相同
							 3); 			//转换后的行数，这样返回的是3×3的矩阵,对应V的最后一列
}
    ...
    const vector<cv::Point2f> &vP2) //归一化后的点, in current frame
{
	...
	//初始化A矩阵
    cv::Mat A(N,9,CV_32F); // N*9维  32位浮点数

    // 构造矩阵A，将每个特征点添加到矩阵A中的元素
    // 矩阵A为n×9维，Af=0为超定方程组，由对A进行SVD分解来求解f
    for(int i=0; i<N; i++)
    {
        const float u1 = vP1[i].x;
        const float v1 = vP1[i].y;
    ...
    float sigma)                        //估计误差
{
	...
	// Step 1 获取从参考帧到当前帧的单应矩阵的各个元素  H21
    ...
    // 查找卡方分布表（Chi-Square Distribution Table），根据自由度和显著性水平，找到对应的临界值（critical value）。
    const float th = 5.991;

    //信息矩阵，方差平方的倒数  信息矩阵即为协方差矩阵的逆
    ...
    for(int i = 0; i < N; i++)
    {
		...
		// 计算投影归一化坐标  计算由单应矩阵H12变换得到的像素坐标,与参考帧中的像素坐标做差
        // p179 公式10-9
    ...
    cv::Mat &x3D)               //三维点
{
    ...
    x3D = vt.row(3).t();//将行向量转置为列向量  vt是4×4矩阵
	//为了符合其次坐标的形式，使最后一维为1
    // 代码中的x3D.rowRange(0,3)表示选取x3D向量的前3个元素，即齐次坐标的(x,y,z)三个分量。
    // x3D.at<float>(3)表示x3D向量的第4个元素，即齐次坐标的尺度因子s。
 ...
 * cv::Point2f 是 OpenCV 库中的一个类，表示二维浮点坐标系中的一个点。它定义在头文件 opencv2/core/types.hpp 中。
 * cv::KeyPoint 是 OpenCV 库中的一个类，表示图像中的一个关键点。它定义在头文件 opencv2/features2d.hpp
 * cv::KeyPoint 类有四个公共数据成员，分别是 pt、size、angle 和 response，分别表示关键点的坐标、尺度、方向和响应值。它们可以直接访问和修改
 * kp.pt.x  kp.pt.y 来访问特征点的二维坐标
 * 
...
void Initializer::Normalize(const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T)                           //将特征点归一化的矩阵
{
    ...
    // |0   sY -meany*sY|
    // |0   0      1    |
    T = cv::Mat::eye(3,3,CV_32F); //创建一个3x3的32位浮点数的单位矩阵，
    T.at<float>(0,0) = sX;
    T.at<float>(1,1) = sY;
...
{   
    ...
    // 先默认所有匹配点对三角化后的三维空间点都不是good点
    vbGood = vector<bool>(vKeys1.size(),false);
	//重设存储空间坐标的点的大小
    vP3D.resize(vKeys1.size());

    //vP3D.resize(vKeys1.size())会改变vector vP3D 的大小为 vKeys1.size()，并根据需要添加或删除元素以匹配新的大小。
    //而vCosParallax.reserve(vKeys1.size())则只是分配了足够的内存以容纳 vKeys1.size() 个元素，
    //但并没有实际添加任何元素到vector vCosParallax 中。
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        ...
        // 利用三角法恢复三维点p3dC1  上面已经求得了投影矩阵P1P2
        Triangulate(kp1,kp2,	//特征点
					P1,P2,		//投影矩阵
					p3dC1);		//输出，三角化测量之后特征点的空间坐标（在图像1相机坐标系下的坐标）		

        //筛选三角化成功的三维点
		// Step 3 第一关：检查三角化的三维点坐标是否合法（非无穷值）
        // 只要三角测量的结果中有一个是无穷大的就说明三角化失败，跳过对当前点的处理，进行下一对特征点的遍历 
        // isfinite 是一个函数名，它的作用是判断一个数值是否为有限数（finite number）如果数值为有限数，isfinite 函数将返回一个非零值，否则返回零。
		...
		//求取模长，其实就是距离 ,实际意义是计算 normal1 向量的长度
        float dist1 = cv::norm(normal1); // 计算 normal1 的 L2 范数，即欧几里得范数,它的 L2 范数定义为：||x|| = sqrt(sum(xi^2))，其中 i 的取值范围是 1 到 N
        ...
        // !可能导致初始化不稳定
        if(p3dC1.at<float>(2)<=0 && cosParallax<0.99998)  //  问题！！
            continue;
        ...
        vCosParallax.push_back(cosParallax);  // 存放合格三角化点对应的视差
		//存储这个三角化测量后的3D点在世界坐标系下的坐标  世界坐标系与相机1坐标系重合
    ...
    if(nGood>0)
    {
        // 将cos值从小到大排序，注意vCosParallax值越大，视差越小。通常视差越大三角化结果越准确
...
void Initializer::DecomposeE(const cv::Mat &E, cv::Mat &R1, cv::Mat &R2, cv::Mat &t)
{
	...
	//准备存储对本质矩阵进行奇异值分解的结果
    cv::Mat u,w,vt;
    // cv::SVDecomp 是一个函数，它直接对一个矩阵进行奇异值分解，并返回奇异值、左奇异向量和右奇异向量
    // cv::SVD::compute 是 cv::SVD 类的成员函数，需要先创建 cv::SVD 对象，然后对该对象进行初始化并计算奇异值分解。
	//对本质矩阵进行奇异值分解
