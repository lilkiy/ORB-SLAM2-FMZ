...
{
    {
        ...
        // 新建或更新连接权重
        // count 是 map 的一个方法，用于返回特定键值的元素数，即如果键存在则返回 1，否则返回 0
        // 判断该共视关键帧 pKF 在不在 
        if(!mConnectedKeyFrameWeights.count(pKF)) 
            // count函数返回0，说明mConnectedKeyFrameWeights中没有pKF，新建连接
            mConnectedKeyFrameWeights[pKF]=weight;
        else if(mConnectedKeyFrameWeights[pKF]!=weight) //或者该共视关键帧在，但权重不一致，就更新权重
            // 之前连接的权重不一样了，需要更新
            mConnectedKeyFrameWeights[pKF]=weight;
        else
            return;
...
void KeyFrame::UpdateBestCovisibles()
{
    ...
    for(map<KeyFrame*,int>::iterator mit=mConnectedKeyFrameWeights.begin(), mend=mConnectedKeyFrameWeights.end(); mit!=mend; mit++)
       vPairs.push_back(make_pair(mit->second,mit->first)); //make_pair函数新建一个pair对象
    ...
    // 更新从大到小排列的权重，和mvpOrderedConnectedKeyFrames一一对应
...
void KeyFrame::UpdateConnections()
{
    ...
    map<KeyFrame*,int> KFcounter;  // 存放与当前关键帧共视的关键帧以及相应的权重（只要有共视就存入）
    vector<MapPoint*> vpMP;

    {
        // 获得当前关键帧的所有地图点
        unique_lock<mutex> lockMPs(mMutexFeatures);
        vpMP = mvpMapPoints; //mvpMapPoints存放当前关键帧观测到的地图点的容器
    } 
    ...
    for(vector<MapPoint*>::iterator vit=vpMP.begin(), vend=vpMP.end(); vit!=vend; vit++)
    {
        ...
        map<KeyFrame*,size_t> observations = pMP->GetObservations(); //获取当前地图点的所有观测

        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            // 除去自身，自己与自己不算共视
            // observations是map结构，mit指向observations的元素，first为关键帧，first->mnId为该关键帧的索引
            ...
            // 如果是第一次出现的共视关键帧，则新增一组键值对，如果该共视关键帧出现过，则增加共视地图点个数
            KFcounter[mit->first]++; 
        }
    }
    ...
    KeyFrame* pKFmax=NULL;  // 保存共视程度最大的关键帧
    // 至少有15个共视地图点才会添加共视关系
    int th = 15;

    // vPairs记录与其它关键帧共视帧数大于th的关键帧
    // pair<int,KeyFrame*>将关键帧的权重写在前面，关键帧写在后面方便后面排序
    vector<pair<int,KeyFrame*> > vPairs; // 只存入共视点大于15的共视关键帧
    vPairs.reserve(KFcounter.size());
    // Step 2 找到对应权重最大的关键帧（共视程度最高的关键帧）
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)
    {
        if(mit->second>nmax) // mit->second 共视关键帧的权重，即共视点个数
        {
            nmax=mit->second;
            pKFmax=mit->first;
        }
        ...
        if(mit->second>=th)
        {
            ...
            // make_pair函数用于创建一个pair对象，它接受两个参数，第一个参数为pair对象的第一个元素，第二个参数为pair对象的第二个元素。
            // pair类有两个公共成员变量，分别为first和second，分别用于存储两个不同类型的元素。可以使用make_pair函数创建一个pair对象
            // pair对象通常用于需要将两个不同类型的值组合在一起的情况。例如，可以使用pair来存储一个键值对
            vPairs.push_back(make_pair(mit->second,mit->first));
            // 对方关键帧也要添加这个信息
            // 更新KFcounter中该关键帧的mConnectedKeyFrameWeights
            // 更新其它KeyFrame的mConnectedKeyFrameWeights，更新其它关键帧与当前帧的连接权重
            (mit->first)->AddConnection(this,mit->second);// 被找到的共视关键帧与当前关键帧也要建立链接
        }
    }
    ...
    // vPairs里存的都是相互共视程度比较高（>th）的关键帧和共视权重，接下来由大到小进行排序
    sort(vPairs.begin(),vPairs.end());         // sort函数默认升序排列 即从小到大
    // 将排序后的结果分别组织成为两种数据类型
    list<KeyFrame*> lKFs;
    list<int> lWs;
    for(size_t i=0; i<vPairs.size();i++)
    {
        // push_front 后变成了从大到小顺序 vPairs的结构是先权重后关键帧
        lKFs.push_front(vPairs[i].second); //往列表前面插入，变成从大到小
        lWs.push_front(vPairs[i].first);
    }

    {
        ...
        mvpOrderedConnectedKeyFrames = vector<KeyFrame*>(lKFs.begin(),lKFs.end()); // 共视关键帧中权重从大到小排序后的关键帧
        mvOrderedWeights = vector<int>(lWs.begin(), lWs.end()); // 共视关键帧中权重从大到小排序后的权重

        // Step 5 更新生成树的连接
        // mbFirstConnection是否是第一次生成树，bool变量
        // 判断当前关键帧是否还未加入到生成树
        if(mbFirstConnection && mnId!=0)
        {
            // 初始化该关键帧的父关键帧为共视程度最高的那个关键帧
            //  mvpOrderedConnectedKeyFrames 容器中的第一个元素赋值给 mpParent 变量。 
            // 权重最大的关键帧作为父关键帧,将当前关键帧作为其子关键帧
            mpParent = mvpOrderedConnectedKeyFrames.front();
            // 建立双向连接关系，将当前关键帧作为其子关键帧
            mpParent->AddChild(this);
            mbFirstConnection = false; // 当前关键帧已经加入到生成树
        }
    }
}
...
{
    {
        ...
        {
            mbNotErase = false;// 剥夺其不被删除的豁免权
        }
    }

    // mbToBeErased：删除之前记录的想要删但时机不合适没有删除的帧
    if(mbToBeErased)  //如果该关键帧曾被豁免过，那么现在直接执行删除操作
    {
        SetBadFlag();
    }
}
...
void KeyFrame::EraseConnection(KeyFrame* pKF)
{
    ...
    {
        unique_lock<mutex> lock(mMutexConnections);
        if(mConnectedKeyFrameWeights.count(pKF)) //判断该关键帧是否在共视容器中，是return 1
        {
