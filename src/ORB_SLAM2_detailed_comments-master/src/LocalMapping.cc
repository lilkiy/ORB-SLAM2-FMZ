...
// 线程主函数
// 该函数内部是一个死循环,每3毫秒查询一次当前线程缓冲队列mlNewKeyFrames.若查询到了待处理的新关键帧,就进行查询
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        // 设置不接收关键帧 设置当前LocalMapping线程处于建图状态,不愿意接受Tracking线程传来的关键帧
        SetAcceptKeyFrames(false);   // 设置 LocalMapping线程是否愿意接收Tracking线程传来的新关键帧

        // Check if there are keyframes in the queue
        // 等待处理的关键帧列表不为空
        if(CheckNewKeyFrames())  // 查看缓冲队列mlNewKeyFrames内是否有待处理的新关键帧 返回bool
        {
            ...
            {
                // Local BA  局部BA优化当前帧的局部地图
                // Step 6 当局部地图中的关键帧大于2个的时候进行局部地图的BA
                if(mpMap->KeyFramesInMap()>2)  // 获取当前地图中的关键帧个数
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    // 该关键帧队列是Tracking线程向LocalMapping中插入的关键帧组成
    {
        ...
        mpCurrentKeyFrame = mlNewKeyFrames.front();  // 取该缓冲队列中的第一帧为当前关键帧
        // 取出最前面的关键帧后，在原来的列表里删掉该关键帧
        mlNewKeyFrames.pop_front();  // 移除并删除该列表中的第一个元素
    }
    ...
    const vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches(); // 获取当前关键帧中特征点对应的地图点
    // 对当前处理的这个关键帧中的所有的地图点展开遍历
    for(size_t i=0; i<vpMapPointMatches.size(); i++)
    {
        MapPoint* pMP = vpMapPointMatches[i];
        if(pMP)  // 如果该特征点对应的地图点有效
        {
            if(!pMP->isBad())  // 且不是坏点
            {
                // 通过检查地图点中是否有对关键帧的观测来判断该地图点是否是新生成的.
                if(!pMP->IsInKeyFrame(mpCurrentKeyFrame)) // 判断该地图点是否是当前关键帧中新生成的.
                {
                    // 该地图点是跟踪本关键帧时匹配得到的，则是之前关键帧中创建的地图点,只需添加其对当前帧的观测即可,在地图点中加入对当前关键帧的观测
                ...
                {
                    // 该地图点是跟踪本关键帧时新生成的,将其加入容器mlpRecentAddedMapPoints待筛选
                    ...
                    mlpRecentAddedMapPoints.push_back(pMP);  //将新增地图点存储到待检查的地图点列表
                }
            }
        }
    }    
    ...
    // 当关键帧的地图点发生变化时，则需要更新和它相连关键帧之间的联系
...
void LocalMapping::MapPointCulling()
{
    ...
    const unsigned long int nCurrentKFid = mpCurrentKeyFrame->mnId; // 获取当前关键帧的索引
    ...
    while(lit!=mlpRecentAddedMapPoints.end())
    {
        ...
        else if(pMP->GetFoundRatio()<0.25f)
        {
            ...
            pMP->SetBadFlag();// 告知可以观测到该MapPoint的Frame，该MapPoint已被删除
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
        // mnFirstKFid创建该MapPoint的关键帧ID
        // 从建立该点开始，到现在已经经过了两个关键帧，但观测到该点的相机数目小于阈值
...
/**
 * @brief 用当前关键帧与相邻关键帧通过三角化产生新的地图点，使得跟踪更稳（单目和双目生成地图点的条件不同）
 * 
 */
...
void LocalMapping::SearchInNeighbors()
{
    ...
    for(vector<KeyFrame*>::const_iterator vit=vpNeighKFs.begin(), vend=vpNeighKFs.end(); vit!=vend; vit++)
    {
        KeyFrame* pKFi = *vit;
        // 没有和当前帧进行过融合的操作
        // 当前一级共视关键帧是坏的或已经和当前关键帧融合过了
        if(pKFi->isBad() || pKFi->mnFuseTargetForKF == mpCurrentKeyFrame->mnId)
            continue;
        // 加入一级相邻关键帧    
        vpTargetKFs.push_back(pKFi);
        // 标记已经加入
        // 标记该一级共视关键帧是与当前关键帧进行融合
    ...
    vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches(); // 获取当前帧中特征点对应的地图点
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++)
    {
        ...
        matcher.Fuse(pKFi,vpMapPointMatches);  // 将地图点投影到关键帧中进行匹配和融合
    }
    ...
    {
        KeyFrame* pKFi = *vitKF;
        vector<MapPoint*> vpMapPointsKFi = pKFi->GetMapPointMatches();  // 获取共视关键帧对应的地图点
        ...
        for(size_t i=0, iend=vpMapPoints.size(); i<iend; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    if(pMP->Observations()>thObs) //观测到该地图点的关键帧数目
                    {
                        const int &scaleLevel = pKF->mvKeysUn[i].octave;//获取二维特征点所在的金字塔层级
