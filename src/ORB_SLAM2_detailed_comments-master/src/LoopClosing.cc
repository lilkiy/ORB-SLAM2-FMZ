...
{
    {
        ...
        // unique_lock的构造函数接受一个mutex作为参数，并且通过lock()函数尝试锁定该mutex。
        //如果mutex已经被另一个线程锁定，则当前线程会被阻塞，直到mutex可用为止。
        //这里的mutex是一个用于同步回环检测队列的互斥锁。只有当当前线程成功锁定了该mutex后，才能够访问回环检测队列。
        // 在unique_lock对象被销毁时，它会自动调用unlock()函数来释放mutex的锁定状态。这样可以确保在任何情况下都会正确地释放mutex的锁定状态，从而避免死锁和其他线程同步问题的出现。
        unique_lock<mutex> lock(mMutexLoopQueue);
        // 从队列头开始取，也就是先取早进来的关键帧
        mpCurrentKF = mlpLoopKeyFrameQueue.front(); // mlpLoopKeyFrameQueue一个队列, 其中存储了参与到回环检测的关键帧
        // 取出关键帧后从队列里弹出该关键帧
        // pop_front()函数用于从队列的头部弹出一个元素，并将其从队列中删除。
    ...
    {
        mpKeyFrameDB->add(mpCurrentKF);  // 将当前关键帧添加进关键帧数据库
        mpCurrentKF->SetErase(); //准备删除当前的这个关键帧,表示不进行回环检测过程 注意该函数并不是真正的删除操作
        return false;
    }
    ...
    // Step 3：遍历当前回环关键帧所有连接/共视（>15个共视地图点）关键帧，计算当前关键帧与每个共视关键的bow相似度得分，并得到最低得分minScore
    const vector<KeyFrame*> vpConnectedKeyFrames = mpCurrentKF->GetVectorCovisibleKeyFrames();// 得到所有与该关键帧连接（共视）的关键帧(已按权值排序)
    const DBoW2::BowVector &CurrentBowVec = mpCurrentKF->mBowVec; //获取当前关键帧的词袋向量
    float minScore = 1;
    for(size_t i=0; i<vpConnectedKeyFrames.size(); i++)
    {
        ...
        float score = mpORBVocabulary->score(CurrentBowVec, BowVec); // score是第三方库中的函数
        // 更新最低得分
        if(score<minScore)
            minScore = score;
    }
    ...
    // 如果没有闭环候选帧，返回false
    if(vpCandidateKFs.empty()) //当前帧不包含任何候选关键帧
    {
        mpKeyFrameDB->add(mpCurrentKF);
        mvConsistentGroups.clear(); //前一关键帧的闭环候选关键帧组（上次闭环检测到的连续组链）清空
    ...
    vector<ConsistentGroup> vCurrentConsistentGroups; // 存储本次闭环检测时的连续组链信息
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        // spCandidateGroup本次闭环检测的子候选组
        set<KeyFrame*> spCandidateGroup = pCandidateKF->GetConnectedKeyFrames(); // 得到与该关键帧连接的关键帧(没有排序的)
        // 把自己也加进去
        ...
        for(size_t iG=0, iendG=mvConsistentGroups.size(); iG<iendG; iG++)
        {
            ...
            {
                if(sPreviousGroup.count(*sit)) // sPreviousGroup.count(*sit) 表示在sPreviousGroup容器中查找sit迭代器指向的关键帧是否存在，如果存在，返回值为1，否则返回0
                {
            ...
            if(bConsistent)
            {
                ...
                if(!vbConsistentGroup[iG])
                {
                    ...
                    //  将当前子候选组和对应的连续长度打包在一起，存入本次闭环检测的连续组链中
...
bool LoopClosing::ComputeSim3()
{
    ...
    const int nInitialCandidates = mvpEnoughConsistentCandidates.size();// 所有达到足够连续数的关键帧组

    // We compute first ORB matches for each candidate
    // 对每个候选关键帧都计算orb特征点匹配项，如果特征点匹配项大于一定阈值，则构造sim3求解器
    // If enough matches are found, we setup a Sim3Solver
    ORBmatcher matcher(0.75,true); //新建一个匹配器对象
    ...
    for(int i=0; i<nInitialCandidates; i++)
    {
        ...
        pKF->SetNotErase(); // 设置该关键帧不被剔除

        // 如果候选帧质量不高，直接PASS
        if(pKF->isBad())
        {
            vbDiscarded[i] = true; //放弃索引为i的闭环候选关键帧
            continue;
        }
...
void LoopClosing::CorrectLoop()
{
    ...
    //停止原因：局部地图线程和全局BA线程都是在后端线程池中运行的，它们主要负责处理新的关键帧、地图点以及进行全局优化。
    //当进行回环检测和位姿校正时，由于涉及到地图的修改和优化，为了避免冲突和歧义，需要先停止局部地图线程和全局BA线程的运行，等回环检测和位姿校正完成后再启动它们。
    //这样可以保证地图的一致性和正确性，并避免多个线程同时修改地图导致的不可预测的错误。
    ...
    // 一直等到局部地图线程结束再继续
    // isStopped()是该类的一个成员函数，用于检查局部建图线程是否已经停止。
    // 如果局部建图线程没有停止，那么就通过std::this_thread::sleep_for()函数暂停当前线程一段时间，等待局部建图线程停止。
    // 循环等待时采用了一定的时间间隔，以避免 CPU 占用过高
    ...
    // 当前帧与世界坐标系之间的Sim变换mg2oScw在ComputeSim3函数中已经确定并优化，
    ...
    // 对地图点操作
    {
        ...
        {
            // 取出当前关键帧连接关键帧
            KeyFrame* pKFi = mit->first; //获取改位姿对应的共视关键帧
            // 取出经过位姿传播后的Sim3变换
            g2o::Sim3 g2oCorrectedSiw = mit->second;
            g2o::Sim3 g2oCorrectedSwi = g2oCorrectedSiw.inverse(); //取逆
            // 取出当前共视关键帧对应的未经过位姿传播的Sim3变换
            g2o::Sim3 g2oSiw =NonCorrectedSim3[pKFi];

            vector<MapPoint*> vpMPsi = pKFi->GetMapPointMatches(); //获取该共视关键帧中特征点对应的地图点
            // 遍历待矫正共视关键帧中的每一个地图点
            for(size_t iMP=0, endMPi = vpMPsi.size(); iMP<endMPi; iMP++)
            {
                ...
                cv::Mat P3Dw = pMPi->GetWorldPos();//获取当前地图点在世界坐标系下的坐标
                // 地图点世界坐标系下坐标
                // 将P3Dw的类型从Mat转换为Matrix<double,3,1>
                // gen中的向量类型为Eigen::Matrix，而ORB-SLAM2中的点类型为cv::Mat。因此，需要将cv::Mat类型的点转换为Eigen::Matrix类型的向量，以便进行矩阵和向量计算。
                ...
                cv::Mat cvCorrectedP3Dw = Converter::toCvMat(eigCorrectedP3Dw); // 类型转换
                pMPi->SetWorldPos(cvCorrectedP3Dw); // 设置地图点为修正后的坐标
                // 记录矫正该地图点的关键帧id，防止重复
                pMPi->mnCorrectedByKF = mpCurrentKF->mnId; //这里为当前关键帧而非共视关键帧
                // 记录该地图点所在的关键帧id 
                pMPi->mnCorrectedReference = pKFi->mnId; // 这里为共视关键帧
    ...
    // CorrectedSim3：存放矫正后当前关键帧的共视关键帧在世界坐标系下Sim3 变换
    ...
    // Step 7：添加当前帧与闭环匹配帧之间的边（这个连接关系不优化）
    // 它在下一次的本质图优化里面使用
    mpMatchedKF->AddLoopEdge(mpCurrentKF);//给当前关键帧添加回环边，回环边连接了形成闭环关系的关键帧
