...
void MapPoint::SetBadFlag()
{
    ...
    for(map<KeyFrame*,size_t>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        // 告诉可以观测到该MapPoint的KeyFrame，该MapPoint被删了 删除关键帧对当前地图点的观测
        pKF->EraseMapPointMatch(mit->second); //(mit->second 为该地图点在该关键帧中的索引
    }
    // 擦除该MapPoint申请的内存
    mpMap->EraseMapPoint(this); // 在地图类上注册删除当前地图点
}
...
{
    // 同一个地图点则跳过
    if(pMP->mnId==this->mnId)  // 比对地图点的全局id
        return;
    ...
    map<KeyFrame*,size_t> obs;
    {
        ...
        nfound = mnFound;
        //指明当前地图点已经被指定的地图点替换了
        mpReplaced = pMP;  // mpReplaced是MapPoint类型的指针变量，保存了新地图点的地址
    }
    ...
    for(map<KeyFrame*,size_t>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        if(!pMP->IsInKeyFrame(pKF)) //判断新地图点有没有别关键帧观测到，这里的关键帧为所有能观测到原地图点的关键帧
        {   
            // 该关键帧中没有对"要替换本地图点的地图点"的观测
            // 替换关键帧中对应id的地图点
            pKF->ReplaceMapPointMatch(mit->second, pMP);// 让KeyFrame用pMP替换掉原来的MapPoint
            // 向新地图点中添加观测，pKF是指向观测该MapPoint对象的关键帧KeyFrame对象的指针
        ...
        else
        {
            ...
            pKF->EraseMapPointMatch(mit->second);  // 删除关键帧对当前地图点的观测，而且不用添加新的观测，因为本身就观测到了要替代的新地图点
        }
    }
...
/**
 * @brief 计算地图点最具代表性的描述子，具有最小距离中值的描述子
 *
...
void MapPoint::ComputeDistinctiveDescriptors()
{
    ...
    {
        unique_lock<mutex> lock1(mMutexFeatures);
        if(mbBad)  // 变量mbBad用来表征当前地图点是否被删除 为坏点标记
            return;
        observations=mObservations; // 该地图点没有被删除则获取其所有观测
    }

    if(observations.empty())  // 该地图点没有被关键帧观测到
        return;

    vDescriptors.reserve(observations.size()); //存储观测到该地图点的所有关键帧对应的特征描述子

    // Step 2 遍历观测到该地图点的所有关键帧，对应的orb描述子，放到向量vDescriptors中
    // C++中的迭代器循环语句，用于遍历observations这个map对象，其中每个键值对代表一个观测到该地图点的关键帧及其在该关键帧中的索引。/
    // 循环中的迭代器mit会依次指向observations中每一个键值对，mit->first表示键，即观测到该地图点的关键帧，mit->second表示值，即该地图点在该关键帧中的索引。
    // 循环终止条件为mit!=mend，即迭代器mit指向的键值对不是最后一个键值对。每次循环结束后，迭代器mit会指向下一个键值对，即mit++。
    ...
    // float Distances[N][N];
	std::vector<std::vector<float> > Distances;  // 相当于定义了一个二维动态数组
	Distances.resize(N, vector<float>(N, 0));  // 大小为N×N的矩阵
    ...
    for(size_t i=0;i<N;i++)
    {
        ...
        // Distances矩阵中第i行存放了第i个描述子和其他描述子的距离
		vector<int> vDists(Distances[i].begin(), Distances[i].end());  // 将距离转换为整数int
    ...
    {
        unique_lock<mutex> lock(mMutexFeatures);
        mDescriptor = vDescriptors[BestIdx].clone();   //获取地图点最具代表性的描述子    
    }
}
 ...
 *  只要地图点本身或关键帧对该地图点的观测发生变化,就应该调用函数MapPoint::UpdateNormalAndDepth()更新其观测尺度和方向信息.
 */
void MapPoint::UpdateNormalAndDepth()
{
    ...
    cv::Mat Pos;
    {
        // unique_lock用于管理互斥量的加锁和解锁操作
        // 当创建unique_lock对象时，会自动对互斥量进行加锁，并在unique_lock对象的作用域结束时自动解锁互斥量，
        // 避免了手动加锁和解锁操作可能出现的问题。mMutexFeatures是一个互斥量对象。
        unique_lock<mutex> lock1(mMutexFeatures);
        unique_lock<mutex> lock2(mMutexPos);
        if(mbBad)
            return;
        ...
        // 地图点的平均观测距离是根据其参考关键帧计算的,创建该地图点的参考帧被设为参考关键帧.
    ...
    for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        ...
        normal = normal + normali/cv::norm(normali); //将所有观测向量归一化为单位向量并累加                    
        n++;
    } 

    cv::Mat PC = Pos - pRefKF->GetCameraCenter();                           // 参考关键帧相机光心指向地图点的向量（在世界坐标系下的表示）
    const float dist = cv::norm(PC);                                        // 该点到参考关键帧相机光心的距离
...

    {
        ...
        //mfMinDistance用最大层的特征点之间的距离除以尺度因子得到，从而得到mfMinDistance。
        //mfMinDistance的作用是控制特征点的密度和分布，确保特征点不会过于密集或过于稀疏。
        // 可以根据地图点在不同层的尺度不同，计算出其在不同层对应的最大观测距离。
        mfMaxDistance = dist*levelScaleFactor;                              // 观测到该点的距离上限
        mfMinDistance = mfMaxDistance/pRefKF->mvScaleFactors[nLevels-1];    // 观测到该点的距离下限
        mNormalVector = normal/n;                                           // 获得地图点平均的观测方向向量
    }
}
...
/**
 * @brief 预测地图点对应特征点所在的图像金字塔尺度层数  根据某地图点到某帧的观测深度估计其在该帧图片金字塔上的层级,
 * 
...
int MapPoint::PredictScale(const float &currentDist, KeyFrame* pKF)
{
    ...
    // 取对数
    int nScale = ceil(log(ratio)/pKF->mfLogScaleFactor); // pKF->mfLogScaleFactor相当于对金字塔尺度因子s对对数  log(s)
    if(nScale<0)
        nScale = 0;
    else if(nScale>=pKF->mnScaleLevels)
        nScale = pKF->mnScaleLevels-1;

    return nScale; //返回金字塔的层级数
}
