...
void Tracking::Track()
{
    ...
    // 语句中的lock对象获取了mpMap指向的容器对象的互斥量mMutexMapUpdate的所有权，并对该对象进行加锁。
    // 语句中的lock对象获取了mpMap指向的容器对象的互斥量mMutexMapUpdate的所有权，并对该对象进行加锁。
    unique_lock<mutex> lock(mpMap->mMutexMapUpdate);

    // Step 1：地图初始化
    if(mState==NOT_INITIALIZED)  //若还没初始化,则尝试初始化
    {
        ...
        //这个状态量在上面的初始化函数中被更新
        if(mState!=OK)  // 初始化成功后状态变为ok
            return;
    }
    else  // 已经初始化了就进行跟踪(或重定位)。地图的初始化只需要执行一次
    {
        ...
        // mbOnlyTracking等于false表示正常SLAM模式（定位+地图更新），mbOnlyTracking等于true表示仅定位模式，不更新地图
        // tracking 类构造时默认为false。在viewer中有个开关ActivateLocalizationMode，可以控制是否开启mbOnlyTracking
        if(!mbOnlyTracking) //如果在slam模式下
        {
            ...
            if(mState==OK) // 跟踪成功
            {
                ...
                // 参考关键帧跟踪的应用场景：地图刚初始化之后；恒速模型跟踪失败后
        ...
        if(bOK)
        {
            ...
            for(int i=0; i<mCurrentFrame.N; i++)
            {
                ...
                if(pMP)
                    if(pMP->Observations()<1) // 如果该地图点存在但无法被观测到
                    {
    ...
    {
        // 计算相对姿态Tcr = Tcw * Twr, Twr = Trw^-1  //当前帧相对于参考关键帧的位姿变换
        cv::Mat Tcr = mCurrentFrame.mTcw*mCurrentFrame.mpReferenceKF->GetPoseInverse();
        //保存各种状态
        mlRelativeFramePoses.push_back(Tcr);
        mlpReferences.push_back(mpReferenceKF);  /// 参考关键帧  mlpReferences为存放参考关键帧的列表
        mlFrameTimes.push_back(mCurrentFrame.mTimeStamp); // 所有帧的时间戳
        mlbLost.push_back(mState==LOST);  //是否跟丢的标志
    }
    else
    {
        // This can happen if tracking is lost
        // 如果跟踪失败，则相对位姿使用上一次值  back()获取最后一个元素
 ...
 * Step 3：在mInitialFrame与mCurrentFrame中找匹配的特征点对(单目初始化参考帧和当前帧)
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        {
            // 初始化需要两帧，分别是mInitialFrame，mCurrentFrame 单目初始化参考帧和当前帧
            mInitialFrame = Frame(mCurrentFrame); //将当前帧作为单目初始化的第一帧，即参考帧
            // 用当前帧更新上一帧
            mLastFrame = Frame(mCurrentFrame);
            // mvbPrevMatched  记录"上一帧"所有特征点，存放单目初始化参考帧中的特征点
            ...
            // 由当前帧构造初始器 sigma:1.0 iterations:200  测量误差和RANSAC迭代次数
            // 使用 "new" 关键字在堆上分配内存来创建一个新对象。
            mpInitializer =  new Initializer(mCurrentFrame,1.0,200);

            // 初始化为-1 表示没有任何匹配。这里面存储的是匹配的点的id
            // mvIniMatches初始化阶段中,当前帧中的特征点和参考帧中的特征点的匹配关系
            fill(mvIniMatches.begin(),mvIniMatches.end(),-1);  //将 mvIniMatches 容器中所有元素的值都设为 -1。

            return;
        }
    }
    else    //如果单目初始化器已经被创建。即已经有了单目初始化参考帧
    {
        // Try to initialize
        ...
        {
            delete mpInitializer;  //若上一帧特征点足够,但当前帧特征点太少,则匹配失败,删除初始化器
            // 将 mpInitializer 变量或指针设置为一个空指针，即没有指向任何对象的指针。
            // 删除初始化器对象后 ，释放其内存
        ...
        // 此时满足连续两帧图像的特征点数目>100，可以进行特征匹配
        // Step 3 在mInitialFrame与mCurrentFrame中找匹配的特征点对
        // 创建orb特征匹配器对象，并调用其构造函数进行初始化
        ORBmatcher matcher(
        ...
        if(nmatches<100)
        {
            delete mpInitializer; //删除初始化器对象并释放内存
            mpInitializer = static_cast<Initializer*>(NULL);
            return;
        }
        ...
        // Step 5 进行单目初始化：通过H模型或F模型进行单目初始化，得到两帧间相对运动、初始MapPoints
        // 括号内返回的是当前帧是否能成功初始化
            ...
            vbTriangulated))    //以及对应于mvIniMatches来讲,其中哪些点被三角化了
        {
            ...
            for(size_t i=0, iend=mvIniMatches.size(); i<iend;i++)
            {
                ...
                {
                    mvIniMatches[i]=-1;  // 一组特征点存在匹配关系，但没有成功三角化
                    nmatches--; // 减少成功匹配点对数目
                }
            }

            // Set Frame Poses 创建初始化地图
            // Step 7 将初始化的第一帧mInitialFrame作为世界坐标系，因此第一帧变换矩阵为单位矩阵
            mInitialFrame.SetPose(cv::Mat::eye(4,4,CV_32F));  // 用变换矩阵Tcw设置参考帧的相机位姿
            // 由Rcw和tcw构造Tcw,并赋值给mTcw，mTcw为世界坐标系到相机坐标系的变换矩阵
            // cv::Mat::eye() 是 OpenCV 中的一个函数，用于创建一个单位矩阵
            cv::Mat Tcw = cv::Mat::eye(4,4,CV_32F);  // 4×4的32位浮点类型单位矩阵 齐次
            // Rcw tcw 是计算得到的相机间的变换位姿，由于第一帧位姿为单位矩阵，因此该相对位姿就是第二帧相对世界坐标系的位姿
            ...
            // mvIniP3D是cv::Point3f类型的一个容器，是个存放3D点的临时变量，单目初始化后得到的成功三角化的点
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // Step 1 将初始关键帧,当前关键帧的描述子转为BoW  计算两个关键帧的词袋
    pKFini->ComputeBoW();
    pKFcur->ComputeBoW();

    // Insert KFs in the map
    // Step 2 将关键帧插入到地图     可以将当前的关键帧和之前的关键帧进行匹配，并更新地图中的状态。
    // mpMap为地图句柄 mpMap 是一个指向地图对象的指针
    // 在ORB-SLAM2中，地图由一些关键帧和地图点构成。
    mpMap->AddKeyFrame(pKFini);
    ...
    //  具体解释：i表示帧1中关键点的索引值，mvMatches12[i]的值为帧2的关键点索引值,没有匹配关系的话，vMatches12[i]值为 -1
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        // 没有匹配，跳过
        // 当一组匹配特征点对未能成功三角化出空间点时，其匹配关系置为-1
        if(mvIniMatches[i]<0)
            continue;

        //Create MapPoint.
        // 用三角化点初始化为空间点的世界坐标
        // mvIniP3D存放单目初始化阶段三角化得到的空间点坐标
        ...
        // 向关键帧中添加其观测到的地图点
        // i为参考帧中特征点的索引，mvIniMatches[i]中存放的是在当前帧中和i匹配的特征点索引
        ...
        //mvpMapPoints存放当前关键帧观测到的地图点的容器
        // mvIniMatches[i]为当前关键帧中三角化得到此地图点的特征点索引
        mCurrentFrame.mvpMapPoints[mvIniMatches[i]] = pMP;
        mCurrentFrame.mvbOutlier[mvIniMatches[i]] = false;  // 外点标记 false为非外点
    ...
    // Update Connections  基于观测到的地图点,更新关键帧共视图
    // Step 3.3 更新关键帧间的连接关系
    // 在3D点和关键帧之间建立边，每个边有一个权重，边的权重是该关键帧与当前帧公共3D点的个数
    // 通过不断地观测来不断地构建地图,
    // 当新的地图点被成功生成并添加到地图中时，我们需要更新已经存在的关键帧和地图点之间的链接关系，
    // 生成地图点后需要更新关键帧的链接关系，以便能够更准确地估计相机的位姿，并使地图点更好地分布在整个地图中。
    ...
    // 因为在单目SLAM中，缺乏深度信息使得无法确定相机与场景的真实尺度关系，因此需要一种方式来将不同深度下的视觉信息统一到同一尺度空间。
    // 中值深度是场景深度的一个合理估计值，它既能够涵盖场景的整体深度范围，同时也能够过滤掉深度异常值
    // 为什么是 pKFini 而不是 pKCur ? 答：都可以的，内部做了位姿变换了
    float medianDepth = pKFini->ComputeSceneMedianDepth(2);  //评估当前关键帧场景深度，q=2表示取中值
    ...
    // Step 6 将两帧之间的变换归一化到平均深度1的尺度下  将变换矩阵T归一化
    // Scale initial baseline
    cv::Mat Tc2w = pKFcur->GetPose();
    // x/z y/z 将z归一化到1 
    Tc2w.col(3).rowRange(0,3) = Tc2w.col(3).rowRange(0,3)*invMedianDepth;
    pKFcur->SetPose(Tc2w);  // 更新当前帧的位姿

    // Scale points
    // Step 7 把3D点的尺度也归一化到1
    // 为什么是pKFini? 是不是就算是使用 pKFcur 得到的结果也是相同的? 答：是的，因为是同样的三维点
    vector<MapPoint*> vpAllMapPoints = pKFini->GetMapPointMatches();  // 获取该关键帧的所有地图点
    for(size_t iMP=0; iMP<vpAllMapPoints.size(); iMP++)
    {
        if(vpAllMapPoints[iMP])
        {
            MapPoint* pMP = vpAllMapPoints[iMP];
            pMP->SetWorldPos(pMP->GetWorldPos()*invMedianDepth);  // 设置世界坐标系下的地图点坐标
        }
    }

    //  Step 8 将关键帧插入局部地图，更新归一化后的位姿、局部地图点，维护上一个关键帧和局部关键帧列表
    mpLocalMapper->InsertKeyFrame(pKFini);
    mpLocalMapper->InsertKeyFrame(pKFcur);

    mCurrentFrame.SetPose(pKFcur->GetPose()); //更新归一化后的位姿
    mnLastKeyFrameId=mCurrentFrame.mnId;  //上一个关键帧的id
    ...
    // mvpLocalMapPoints 局部地图点列表
    mvpLocalMapPoints=mpMap->GetAllMapPoints();
    mpReferenceKF = pKFcur;  // 当前帧就是局部地图的参考关键帧
    //也只能这样子设置了,毕竟是最近的关键帧
    mCurrentFrame.mpReferenceKF = pKFcur;

    mLastFrame = Frame(mCurrentFrame); //上一帧

    mpMap->SetReferenceMapPoints(mvpLocalMapPoints); // 设置参考地图点

    //mpMapDrawer 是一个地图可视化对象的指针，用于绘制地图及相关的可视化元素。
    mpMapDrawer->SetCurrentCameraPose(pKFcur->GetPose());

    //mpMap 是一个地图对象的指针，用于维护整个地图的数据结构。
    //  mvpKeyFrameOrigins 保存最初始的关键帧
    mpMap->mvpKeyFrameOrigins.push_back(pKFini);

    mState=OK;// 初始化成功，至此，初始化过程完成  设置跟踪状态
}
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    mCurrentFrame.ComputeBoW();  // 计算当前帧的词袋

    // We perform first an ORB matching with the reference keyframe
    // If enough matches are found we setup a PnP solver
    ORBmatcher matcher(0.7,true); // 最优和次优评分的比例   是否检查方向
    vector<MapPoint*> vpMapPointMatches;

    // Step 2：通过词袋BoW加速当前帧与参考帧之间的特征点匹配
    // 通过词袋，对关键帧的特征点进行跟踪
    // SearchByBoW函数是基于词袋（bag of words，BoW）模型实现的地图点匹配算法
    // vpMapPointMatches 记录匹配成功后当前帧的每个特征点对应的地图点（参考关键帧中的）
    ...
    // Step 3:将上一帧的位姿态作为当前帧位姿的初始值，可以加速收敛
    // mvpMapPoints存放当前帧中特征点对应的MapPoint，即赋值给当前帧的地图点容器
    mCurrentFrame.mvpMapPoints = vpMapPointMatches;
    mCurrentFrame.SetPose(mLastFrame.mTcw); // 用上一次的Tcw设置初值，在PoseOptimization可以收敛快一些

    // Step 4:通过优化3D-2D的重投影误差来获得位姿  优化当前帧的位姿
...
/**
 * @brief 更新上一帧位姿，对于双目或rgbd相机，在上一帧中生成临时地图点
...
void Tracking::UpdateLastFrame()
{
    ...
    // mlRelativeFramePoses中存放了相邻的关键帧之间的相对位姿变换  .back获取最后一个元素
    cv::Mat Tlr = mlRelativeFramePoses.back(); // 获取参考关键帧到上一帧的相对位姿

    // 将上一帧的世界坐标系下的位姿计算出来
    // l:last, r:reference, w:world
    // Tlw = Tlr*Trw 
    // pRef->GetPose() h获取参考关键帧的位姿
    ...
    for(int i=0; i<mLastFrame.N;i++)  // mLastFrame.N获取上一帧中的关键点个数
    {
    ...
    int nPoints = 0;  //上一帧中全部地图点的数目
    for(size_t j=0; j<vDepthIdx.size();j++)  // 遍历上一帧中所有特征点
    {
        int i = vDepthIdx[j].second;  //i为特征点的id

        bool bCreateNew = false;  // 默认不需要创建新的地图点

        // 如果这个点对应在上一帧中的地图点没有,或者创建后就没有被观测到,那么就生成一个临时的地图点
        MapPoint* pMP = mLastFrame.mvpMapPoints[i]; // 获取索引为i的特征点对应的地图点，判断是否被创建为地图点
        if(!pMP)
            bCreateNew = true;  // 需要创建临时地图点
        else if(pMP->Observations()<1)      // pMP->Observations()获取当前地图点被观测到的相机数目 单目+1，双目/RGBD+2
        {
        ...
        if(bCreateNew)
        {
            ...
            // 反投影到世界坐标系中
            cv::Mat x3D = mLastFrame.UnprojectStereo(i);  // 反投影得到该特征点对应的3D点坐标
            // 由3D点坐标创建地图点
        ...
        if(vDepthIdx[j].first>mThDepth && nPoints>100)  // 地图点的深度大于阈值或地图点数目足够多
            break;
    }
}
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // 将上一帧跟踪的地图点投影到当前帧，并且搜索匹配点。用于跟踪前一帧,返回成功匹配的数量
    ...
    // Step 4：利用3D-2D投影关系，优化当前帧位姿，只优化位姿，不优化地图点
    Optimizer::PoseOptimization(&mCurrentFrame);
    ...
    for(int i =0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            if(mCurrentFrame.mvbOutlier[i])
            {
                ...
                mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);
                mCurrentFrame.mvbOutlier[i]=false;
                pMP->mbTrackInView = false;  // 已经和当前帧经过匹配（TrackReferenceKeyFrame，TrackWithMotionModel）但在优化过程中认为是外点 则为false
                pMP->mnLastFrameSeen = mCurrentFrame.mnId;
                nmatches--;
            }
            else if(mCurrentFrame.mvpMapPoints[i]->Observations()>0)
                // 累加成功匹配到的地图点数目
                nmatchesMap++; // 如果该地图点不是外点而且能被观测到
        }
    }    

    if(mbOnlyTracking)  // 标记当前系统是处于SLAM状态还是纯定位状态
    {
...
bool Tracking::TrackLocalMap()
{
    ...
    // 函数Tracking::UpdateLocalMap()依次调用函数Tracking::UpdateLocalKeyFrames()更新局部关键帧列表mvpLocalKeyFrames
    // 和函数Tracking::UpdateLocalPoints()更新局部地图点列表mvpLocalMapPoints.
    UpdateLocalMap();
    ...
    // 遍历当前帧的所有关键点
    for(int i=0; i<mCurrentFrame.N; i++)  // N: NUM OF KEYPOINTS
    {
        if(mCurrentFrame.mvpMapPoints[i]) // 如果该关键点存在匹配的地图点
        {
            // 由于当前帧的地图点可以被当前帧观测到，其被观测统计量加1
            if(!mCurrentFrame.mvbOutlier[i])
            {
                // 找到该点的帧数mnFound 加 1  实际观测到该地图点的帧数+1
                mCurrentFrame.mvpMapPoints[i]->IncreaseFound();
                //查看当前是否是在纯定位过程
                // 如果当前不是在纯定位模式下，也就是说当前帧不是一个关键帧，那么会统计当前帧中的内点数量。
                if(!mbOnlyTracking)
                {
                ...
                // 如果当前在纯定位模式下，也就是说当前帧是一个关键帧，那么在TrackLocalMap()函数中不会进行内点数量的统计，
                // 而是直接将匹配内点计数设置为当前帧跟踪到的地图点数目。这是因为在关键帧中，所有的地图点都会被用来进行位姿估计和地图优化，而不是仅仅考虑内点。
...
void Tracking::CreateNewKeyFrame()
{
    ...
    mpReferenceKF = pKF; // 将创建出的关键帧设为参考关键帧
    ...
    if(mSensor!=System::MONOCULAR)
    {
        // 根据Tcw计算mRcw、mtcw和mRwc、mOw   这行代码的作用是更新当前帧的相机位姿矩阵
        ...
        for(int i=0; i<mCurrentFrame.N; i++)
        {
            float z = mCurrentFrame.mvDepth[i]; // 获取特征点对应的深度
            if(z>0)
            {
        ...
        if(!vDepthIdx.empty())
        {
            ...
            // 处理的近点的个数
            int nPoints = 0; // 地图点总数
            // 遍历当前帧的所有关键点，判断是否需要创建对应的地图点
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                int i = vDepthIdx[j].second;

                bool bCreateNew = false;  // 是否需要创建新是地图点

                // 如果这个点对应在上一帧中的地图点没有,或者创建后就没有被观测到,那么就包装为地图点
                MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];
                // 如果索引为i的特征点存在有效深度值，但没有跟踪到对应的地图点，则需要将该特征点对应的三维点创建为地图点
                if(!pMP)
                    bCreateNew = true;
                else if(pMP->Observations()<1) // 如果该地图点存在但没有被观测到，则清除该地图点并创建新的地图点
                {
                    bCreateNew = true;
                    mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
                }
                ...
                if(bCreateNew)
                {
                    cv::Mat x3D = mCurrentFrame.UnprojectStereo(i); // 通过反投影得到特征点i对应的三维点坐标
                    ...
                    pNewMP->ComputeDistinctiveDescriptors();
                    pNewMP->UpdateNormalAndDepth();
                    mpMap->AddMapPoint(pNewMP); // 将新创建的地图点添加进地图中
                ...
                {
                    // 因为从近到远排序，记录其中不需要创建地图点的个数，说明已经有了地图点
                    nPoints++;
                }
    ...
    // 将创建好的关键帧送入局部建图线程中
    // 局部建图线程负责建立和更新局部地图，包括对新的关键帧进行建图和对地图点进行优化等。
...
void Tracking::SearchLocalPoints()
{
    ...
    for(vector<MapPoint*>::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        // Project (this fills MapPoint variables for matching)s
        // 判断地图点是否在在当前帧视野内
    ...
    if(nToMatch>0)
    {
        ...
        // 投影匹配得到更多的匹配关系
        // 通过投影匹配搜索空间点和关键帧上点的匹配关系，返回匹配的特征点数目
...
void Tracking::UpdateLocalPoints()
{
    ...
    {
        KeyFrame* pKF = *itKF;
        const vector<MapPoint*> vpMPs = pKF->GetMapPointMatches(); // 获取当前关键帧中特征点对应的全部地图点
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    // Step 1：遍历当前帧的地图点，记录所有能观测到当前帧地图点的关键帧，并统计共视程度
    map<KeyFrame*,int> keyframeCounter; // 用于存放当前帧的一级共视关键帧和共视程度（共同观测到的地图点个数）
    for(int i=0; i<mCurrentFrame.N; i++)  // N为关键点个数
    {
        if(mCurrentFrame.mvpMapPoints[i])  // 如果第i个关键点对应的地图点存在
        {
            MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];
            if(!pMP->isBad())  // 如果该地图点是有效地图点
            {
    ...
    {
        KeyFrame* pKF = it->first; // 获取一级共视关键帧
        ...
        // 用该关键帧的成员变量mnTrackReferenceForFrame 记录当前帧的id（mnId）
...
bool Tracking::Relocalization()
{
    ...
    // Step 2：用词袋找到与当前帧相似的候选关键帧集合
    // 在重定位中找到与该帧相似的关键帧构成候选关键帧集合
    ...
    const int nKFs = vpCandidateKFs.size(); // 候选关键帧数目
    ...
    for(int i=0; i<nKFs; i++)
    {
        ...
        // pKF->isBad()的作用是检查当前指向的关键帧是否已经被标记为“坏帧”。
        // 一个关键帧如果被标记为“坏帧”，意味着它不再被用于建图和定位，通常是因为这个关键帧的质量太差（例如视角变化太大，或者匹配点太少等）或者出现了错误（例如跟踪丢失）。
        if(pKF->isBad())
            vbDiscarded[i] = true;
        else
        {
    ...
    // 是否已经找到相匹配的关键帧的标志
    bool bMatch = false;  // 当前帧和候选关键帧集合中的关键帧是否匹配成功的标记，只要和一个关键帧匹配成功就算重定位成功
    ORBmatcher matcher2(0.9,true); // 创建一个ORB特征匹配对象
    ...
    while(nCandidates>0 && !bMatch)
    {
        ...
        for(int i=0; i<nKFs; i++)
        {
            ...
            bool bNoMore;  // 是否继续进行迭代，即达到最大迭代次数的标志
            ...
            if(!Tcw.empty())
            {
                ...
                // EPnP 里RANSAC后的内点的集合
                set<MapPoint*> sFound;  // 存放判断为内点的地图点

                const int np = vbInliers.size();
                //遍历所有内点
                for(int j=0; j<np; j++) //判断第j个点是否是内点
                {
                    if(vbInliers[j])
                    {
                ...
                int nGood = Optimizer::PoseOptimization(&mCurrentFrame); // BA优化当前帧的位姿
                ...
                // 删除外点对应的地图点
                for(int io =0; io<mCurrentFrame.N; io++) // N为关键点数目
                    if(mCurrentFrame.mvbOutlier[io])  //如果索引为io的特征点是外点，那么删除该特征点对应的地图点
    ...
    else
    {
        ...
        // 上一次重定位的那一帧的ID  上一个进行了重定位的图像帧的id
